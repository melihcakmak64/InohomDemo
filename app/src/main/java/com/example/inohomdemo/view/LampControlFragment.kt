...
import com.example.inohomdemo.viewmodel.LampControlViewModel
...
class LampControlFragment : Fragment() {
    ...
    private val viewModel: LampControlViewModel by viewModels()
    ...
    override fun onCreateView(
        ...
        viewModel.updateControlValueResponse.observe(viewLifecycleOwner, Observer { response ->
            if (response != null && response.method.equals("OnEntityUpdated")) {
                Log.d("LampControlFragment", "$response")
            } else {
                Log.e("LampControlFragment", "Authentication failed")
            }
        })
